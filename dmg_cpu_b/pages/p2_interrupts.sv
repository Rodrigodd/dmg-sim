...
module interrupts(
	...
	assign #T_TRI  nela = pola ? ~(~maty) : 1'bz;
	assign #T_TRI  pado = pola ? ~(~nejy) : 1'bz;
	assign #T_TRI  pegy = pola ? ~(~nuty) : 1'bz;
	assign #T_TRI  nabo = pola ? ~(~mopo) : 1'bz;
	assign #T_TRI  rova = pola ? ~(~pavy) : 1'bz;
